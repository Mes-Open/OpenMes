...
class LotPickingServiceTest extends TestCase
{
    ...
    public function test_fefo_picks_earliest_expiry_first(): void
    {
        ...
        $this->assertEqualsWithDelta(50.0, (float) $lotNoExpiry->fresh()->available_qty, 0.0001);
    }
    ...
    public function test_depletes_lot_when_fully_picked(): void
    {
        ...
        $this->assertEqualsWithDelta(0.0, (float) $lot->fresh()->available_qty, 0.0001);
        $this->assertSame(MaterialLot::STATUS_DEPLETED, $lot->fresh()->status);
    }

    public function test_skips_quarantined_lots(): void
    {
        $lot = $this->makeLot('LOT-A', 100);
        $lot->update(['status' => MaterialLot::STATUS_QUARANTINED]);
    ...
    public function test_return_picks_restores_lot_availability_and_status(): void
    {
        ...
        $this->assertEqualsWithDelta(100.0, (float) $lot->fresh()->available_qty, 0.0001);
        $this->assertSame(MaterialLot::STATUS_AVAILABLE, $lot->fresh()->status);
