...
class InspectionCreatesLotTest extends TestCase
{
    ...
    public function test_pass_creates_available_lot_with_full_quantity(): void
    {
        ...
        $this->assertEqualsWithDelta(250.0, (float) $lot->received_qty, 0.0001);
        $this->assertEqualsWithDelta(250.0, (float) $lot->available_qty, 0.0001);
    ...
    public function test_fail_creates_quarantined_lot_with_zero_available(): void
    {
        ...
        $lot = MaterialLot::firstWhere('inspection_id', $insp->id);
        $this->assertNotNull($lot);
        $this->assertSame(MaterialLot::STATUS_QUARANTINED, $lot->status);
        $this->assertEqualsWithDelta(100.0, (float) $lot->received_qty, 0.0001);
        $this->assertEqualsWithDelta(0.0, (float) $lot->available_qty, 0.0001);
    }
