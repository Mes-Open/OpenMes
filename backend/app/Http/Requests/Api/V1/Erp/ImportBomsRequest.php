...
class ImportBomsRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'recipes.*.components.*.material_code' => ['nullable', 'string', 'max:50'],
            'recipes.*.components.*.component_code' => ['nullable', 'string', 'max:50'],
            'recipes.*.components.*.component_kind' => ['nullable', 'in:material,product_type'],
            'recipes.*.components.*.component_template_version' => ['nullable', 'integer', 'min:1'],
            'recipes.*.components.*.extra_data' => ['nullable', 'array'],
            'recipes.*.components.*.foam_grade' => ['nullable', 'string', 'max:100'],
            'recipes.*.components.*.length_mm' => ['nullable', 'numeric', 'gt:0'],
            'recipes.*.components.*.width_mm' => ['nullable', 'numeric', 'gt:0'],
            'recipes.*.components.*.thickness_mm' => ['nullable', 'numeric', 'gt:0'],
