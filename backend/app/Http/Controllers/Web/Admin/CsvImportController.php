...
class CsvImportController extends Controller
{
    ...
    public function index()
    {
        ...
        return Inertia::render('admin/CsvImport', [
            'basePath' => $this->basePath('/csv-import'),
    ...
    public function process(Request $request)
    {
        ...
        $missingRequired = [];
        if (! in_array('order_no', $mappedTargets)) {
            $missingRequired[] = 'order_no';
        }
        if (! in_array('quantity', $mappedTargets)) {
        ...
        if (! empty($missingRequired)) {
            ...
            return redirect()->to($this->sectionRoute('csv-import'))
        ...
        foreach ($rows as $rowData) {
            ...
            try {
                ...
                if ($importWeek) {
                    $workOrderData['week_number'] = $importWeek;
                }
                if ($importMonth) {
                    $workOrderData['month_number'] = $importMonth;
                }
                if ($productionYear) {
                    $workOrderData['production_year'] = $productionYear;
                }
    ...
    private function mapRow(array $rowData, array $mapping, $lines, $productTypes): array
    {
        ...
        foreach ($mapping as $csvColumn => $target) {
            ...
            switch ($target) {
                case 'order_no':
                    if ($value === '') {
                        throw new \Exception('order_no is empty');
                    }
                ...
                case 'line_code':
                    if ($value !== '') {
                        ...
                        if (! $lineId) {
                            throw new \Exception("Line '{$value}' not found");
                        }
                ...
                case 'product_type_code':
                    if ($value !== '') {
                        ...
                        if (! $ptId) {
                            throw new \Exception("Product type '{$value}' not found");
                        }
