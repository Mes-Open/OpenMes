...
class NetRequirementsService
{
    ...
    public function report(Carbon $from, Carbon $to, ?int $lineId = null): array
    {
        ...
        // Component orders (a generated root or its children) carry their BOM frozen
        // in the snapshot; the live template only drives ordinary orders.
        $usesSnapshotBom = fn (WorkOrder $wo) => $wo->component_plan || $wo->root_work_order_id;
        $templates = $this->activeTemplateByProductType(
            $workOrders->reject($usesSnapshotBom)->pluck('product_type_id')->unique()->filter()
        );
        ...
        foreach ($workOrders as $wo) {
            if ($usesSnapshotBom($wo)) {
                $lines = collect($wo->process_snapshot['bom'] ?? [])
                    ->filter(fn ($row) => ! empty($row['material_id']))
                    ->map(fn ($row) => [
                        'material_id' => (int) $row['material_id'],
                        'required_per_unit' => round((float) $row['quantity_per_unit'] * (1 + (float) ($row['scrap_percentage'] ?? 0) / 100), 6),
                    ])
                    ->all();
            } else {
                ...
                $lines = $this->directComponents($template);
            }
