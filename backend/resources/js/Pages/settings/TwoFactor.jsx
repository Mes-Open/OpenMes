...
export default function TwoFactor() {
    ...
    // Recovery codes are one-time. Purge Inertia's history cache so pressing
    // Back can't restore this cached page (with the codes) — a fresh visit to
    // the enable route has no flash and renders without them.
    useEffect(() => {
        ...
        <>
            ...
            <div className="p-6 max-w-2xl mx-auto space-y-6">
                <div>
                    ...
                    <h1 className="mt-3 text-2xl font-bold text-gray-900 dark:text-white">Two-Factor Authentication</h1>
                </div>
...
function RecoveryCodes({ codes }) {
    ...
    return (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 rounded-xl p-5">
            <h2 className="text-sm font-bold text-amber-800 dark:text-amber-300 uppercase tracking-wide">Recovery codes</h2>
            <p className="text-sm text-amber-700 dark:text-amber-400 mt-1">
                Store these somewhere safe. Each code can be used once if you lose access to your authenticator. They won't be shown again.
            </p>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 space-y-4">
            ...
            {qrCodeDataUri && (
                <div className="text-center">
                    <img src={qrCodeDataUri} alt="2FA QR code" className="mx-auto rounded-lg border border-gray-200 dark:border-gray-600" />
                </div>
            )}

            <div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-1">Can't scan? Enter this key manually:</p>
            ...
            <form onSubmit={submit} className="space-y-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                ...
                <button type="submit" disabled={form.processing || form.data.code.length !== 6} className="w-full px-5 py-2.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
                    {form.processing ? 'Verifying…' : 'Enable Two-Factor Authentication'}
                </button>
            </form>
        </div>
    );
    ...
    return (
        <div className="space-y-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 flex items-center gap-3">
                ...
                <div>
                    <h2 className="font-bold text-gray-900 dark:text-white">Two-factor authentication is on</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Your account requires an authentication code at login.</p>
                </div>
            </div>

            <PasswordActionCard
                title="Regenerate recovery codes"
                description="Invalidate your old recovery codes and generate a fresh set."
            ...
            <PasswordActionCard
                ...
                description="Remove 2FA from your account. You'll only need your password to log in."
