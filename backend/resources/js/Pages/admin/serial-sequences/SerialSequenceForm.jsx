 ...
 */
export function serialSequenceInitial(r) {
    return {
        name: r?.name ?? '',
...
/**
 * Create/edit form for Serial sequences — parallel to LotSequenceForm, two
 * modes:
 *  - Pattern: token template ("SN-[date]-[seq]") with a clickable token
 *    palette and a debounced live preview from the server.
 *  - Simple (legacy): prefix / year prefix / suffix.
 */
export default function SerialSequenceForm({ action, method, initial, submitLabel, bare = false, onSuccess, onCancel }) {
    ...
    useEffect(() => {
        ...
        const t = setTimeout(async () => {
            try {
                const r = await fetch('/admin/serial-sequences/preview', {
                    method: 'POST',
        ...
        <form onSubmit={submit} className={bare ? 'space-y-5' : 'bg-om-card rounded-om-sm shadow-sm p-6 max-w-2xl space-y-5'}>
            ...
            <div className="flex items-center gap-3 pt-2">
                ...
                {onCancel ? (
                    ...
                    <Link href="/admin/serial-sequences" className="text-om-muted hover:text-om-ink text-sm">{__('Cancel')}</Link>
                )}
            </div>
        </form>
    );
