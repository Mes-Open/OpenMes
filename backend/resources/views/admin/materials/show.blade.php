                ...
                <tbody class="divide-y divide-gray-200">
                    @foreach($lots as $lot)
                        ...
                        <tr class="{{ $lot->isExpired() ? 'bg-red-50' : '' }}">
                            ...
                            <td class="px-3 py-2 text-gray-500 font-mono text-xs">{{ $lot->supplier_lot_no ?? '—' }}</td>
                            <td class="px-3 py-2 text-right font-mono">{{ number_format($lot->quantity_received, 3) }}</td>
                            <td class="px-3 py-2 text-right font-mono {{ $lot->quantity_available <= 0 ? 'text-gray-400' : 'font-bold' }}">
                                {{ number_format($lot->quantity_available, 3) }}
                            </td>
