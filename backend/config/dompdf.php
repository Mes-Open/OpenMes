...
return [
    ...
    */
    ...
    'options' => [
         ...
         */
        // Narrowed from base_path(): with the whole application readable, any
        // user-controlled string that reached an <img src> or a CSS url() could
        // pull .env, config or storage into the rendered document. Nothing we
        // render loads from disk at all — QR codes are embedded as data: URIs —
        // so the only directory worth reaching is the public one.
        ...
        'allowed_protocols' => [
            'data://' => ['rules' => []],
            // file:// removed on purpose. No view we render reads from disk,
            // so allowing it only leaves a local-file-read primitive lying
            // around for the day someone interpolates user input into a src.
            'http://' => ['rules' => []],
            'https://' => ['rules' => []],
        ],
         ...
         * @var bool
         */
        // Off: this embeds JavaScript into the produced PDF, which some readers
        // (Acrobat) will execute. Our documents are tables of plant data; none
        // of them needs to run code in the reader, and leaving it on keeps a
        // script-injection class alive for no benefit.
