 ...
 * (SHIFT-click), column visibility menu, drag-to-resize headers (min 54px),
 * sticky header inside a capped scroll body, page-level row selection with a
 * bulk-actions toolbar, and pagination. No native twin — web only.
 *
...
export function DataTable({
    ...
    // Grid template shared by header, filter row and body rows. Hidden columns
    // drop out of `visibleCols`; flex columns stretch until manually resized.
    const gridTemplateColumns = [
        ...(enableSelection ? ['38px'] : []),
        ...visibleCols.map((col) =>
            state.columnSizing[col.id] == null && col.columnDef.meta?.flex
                ? 'minmax(140px, 1fr)'
                : `${col.getSize()}px`,
        ),
    ].join(' ');
            ...
            <div className="overflow-hidden rounded-om border border-om-line">
                <div className="overflow-auto" style={{ maxHeight: bodyMaxHeight }}>
                    <div className="sticky top-0 z-[3]">
                        {/* header */}
                        <div
                            className="grid items-center border-b border-om-line2 bg-om-panel px-4 py-[10px]"
                                ...
                                hg.headers.map((header) => {
                                    ...
                                    return (
                                        <span
                                            ...
                                            className={`relative overflow-hidden font-mono text-[9px] tracking-[0.1em] uppercase select-none ${
                        ...
                        {hasFilterRow && (
                            <div
                                className="grid items-center border-b border-om-line2 bg-om-card px-4 py-2"
                                style={{ gridTemplateColumns }}
                            >
                                ...
                                {visibleCols.map((col) => {
                                    ...
                                    if (meta.filter === 'text') {
                                        return (
                                            <span key={col.id} className="overflow-hidden pr-2">
                                                <input
                    ...
                    {pageRows.map((row) => (
                        <div
                            ...
                            className={`grid items-center border-b border-om-line2 px-4 py-3 text-[13.5px] text-om-ink ${
                    ...
                    {total === 0 && (
                        <div className="p-[34px] text-center text-[13.5px] text-om-faint">
                            {emptyLabel}
                        </div>
                    )}
                </div>
