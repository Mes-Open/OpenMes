/**
 * Edit division — re-skinned to the spec form/detail pattern. Mirrors the web
 * admin division form (Code / Name / Description / Active) plus an activate
 * toggle and delete. Hooks, payloads and navigation are unchanged.
 */
...
export function EditDivisionScreen() {
  ...
  if (query.isLoading && !d) return <LoadingState />;
  if (query.isError || !d) return <ErrorState error={query.error} onRetry={query.refetch} />;
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <Text style={styles.h1}>{t('Edit division')}</Text>
      ...
      <View style={styles.danger}>
        <Button
          ...
          onPress={() => toggleMutation.mutate(d.id, { onError: (e: Error) => Alert.alert(t('Failed'), e.message) })}
        />
        <Button
