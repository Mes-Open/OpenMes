...
export function NewProductTypeScreen() {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <Text style={styles.h1}>{t('New product type')}</Text>

      <Field label="Code" value={code} onChangeText={setCode} error={errors.code} required autoCapitalize="characters" autoCorrect={false} mono placeholder="WIDGET-A" />
      <Field label="Name" value={name} onChangeText={setName} error={errors.name} required placeholder="Widget Type A" />
      ...
      <View style={styles.actions}>
        ...
        <View style={{ flex: 1 }} />
        <Button title={t('Save')} onPress={onSave} loading={createMutation.isPending} disabled={createMutation.isPending} />
      </View>
    </ScrollView>
  );
}
