...
### Fixed
- **Supervisors can now create & manage work orders (as the role docs state)**: the Supervisor role already held the `create work orders` ability and `WorkOrderPolicy::create` allowed it, but there was no way to use it — no supervisor create route/controller/button, and the role lacked the `tab:orders` permission needed to reach the admin order pages. Supervisor now gets `tab:orders` (seeder) **and** a native create flow: `GET/POST /supervisor/work-orders` (create/store, authorized via `WorkOrderPolicy`) with a **"+ New Work Order"** button on the supervisor Orders list. Operators and guests remain blocked.
