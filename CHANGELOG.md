...
### Changed
- **Schedule planner (Admin → Schedule) rebuilt from scratch**: the production planner board was replaced with a cleaner, less buggy implementation across all four views (Weekly, Daily, Hourly, Monthly). Work-order cards now use a status-coloured left spine with priority, quantity and live progress; the backlog rail groups unassigned orders by priority with search/sort; an empty slot can be clicked to assign from the backlog; and a bottom edit sheet handles line/date/shift changes. Drag-and-drop scheduling (now built on **react-dnd** instead of raw HTML5 handlers, so dragging a card never reflows the grid), hourly minute-level move/resize with same-line conflict detection (with a "schedule anyway" override), live Electric sync and the live-order tracking panel are preserved, now wired through the existing reschedule/resize endpoints. The weekly grid renders one row per line with a sticky line cell, so a line's read-only maintenance strip extends its own row instead of pushing the rest of the board out of alignment. Internally the controller no longer pre-builds the grid server-side — it ships a flat work-order list and the board computes layout client-side (minute-planned orders still appear on the coarse grid, placed by due date) — which removed ~450 lines of grid/span code. Selects/date inputs use the shared `@openmes/ui` `Dropdown`/`DatePicker`. The whole board uses the OpenMES brand tokens, so it follows dark mode automatically.
...
### Fixed
- **Shift `days_of_week` day-format inconsistency**: shifts are meant to store their active weekdays as ISO integers (1=Mon … 7=Sun) — as the API validation, migration default and `Shift::current()` all assume — but the demo seeder wrote day-name strings (`"monday"`, …), so integer comparisons silently never matched. The result: `Shift::current()` failed to resolve the active shift against seeded data, and any feature reading shift weekdays saw "no shifts". A new `App\Casts\DaysOfWeek` normalizes the column to ISO integers on both read and write (tolerating legacy string rows already in the database), and the seeder now writes integers. The schedule capacity view's available-hours calculation depended on this.
- **Placeholder logo + washed-out Sign-in button**: the login screen, setup-wizard (onboarding) steps, operator touch-UI top bar, and the dev component gallery all showed a placeholder gradient mark + "openmes" text instead of the real brand logo used in the authenticated app shell — they now render `/logo_open_mes.png`. The shared `Button` also faded to a near-white chip when disabled (invisible on white cards, e.g. the disabled Sign-in button on load); disabled/loading now dims the variant's own colour instead, so an accent button stays orange. This also removes a fragile disabled-state class conflict (two `bg-*` utilities resolved by CSS order).
- **OEE report gauges rendered as a single-colour arc**: the full OEE report (Admin → OEE) used a one-colour CSS-arc gauge with the percentage overlapping the fill, instead of the red/yellow/green banded gauge with a needle used on the dashboard (the React port of `<x-oee-gauge>`). Extracted that banded gauge into a shared `components/OeeGauge.jsx` and used it on both the dashboard OEE Overview and the report, so they match again.
- **Slashed zero in mono numbers (web + mobile)**: Geist Mono's default `0` is slashed, which clashed with the rest of the UI on dashboards/KPIs and any mono figure. On the **web** app, `--font-mono` now enables the plain-zero stylistic set (`font-feature-settings: 'ss09' 1`). On **mobile**, where React Native can't toggle that feature reliably across iOS+Android and the mono font is set in ~15 places, the three Geist Mono weights are vendored under `mobile/assets/fonts/` patched so the default `0` glyph is the plain round zero (recipe + OFL in that folder). Also dropped the unused `'Instrument Sans'` name from the web `--font-sans` stack (only Geist is actually loaded).
...
### Added
- **Schedule capacity view (available vs planned hours)**: a new **Capacity** view (Admin → Schedule → Capacity) shows, per resource and per time bucket, how many **available** hours that resource has against how many hours of work are **planned** against it. Two axes: a **line** axis (machine capacity — available from the active shift calendar, line-specific plus global shifts, minus scheduled maintenance) and a **crew** axis (labor capacity — available from each crew's workers' line shifts minus approved absences and crew break windows; planned from logged work activity). Each cell renders a load bar coloured by band (green under 80%, amber 80–100%, red over 100%) with the underlying `planned / available` hours; cells with planned work but **no capacity** are flagged red ("No capacity") rather than muted. **Week/day granularity toggle** and **line/crew axis toggle**. Line planned hours come from minute-level plans when present (distributed across the buckets they span), otherwise a best-effort estimate (batch-step durations → process-snapshot estimates); work orders with no estimable duration are surfaced as an **"N unestimated"** count rather than counted as zero load. On the line axis, **clicking a cell drills into the work orders feeding it and reschedules them inline** (move line/date), reusing the planner's existing update endpoint; the grid recomputes live. New `CapacityService` + `ScheduleCapacityController`, linked from the planner toolbar.
