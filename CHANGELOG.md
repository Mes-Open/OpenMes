...
### Changed
- **Operators granted admin tabs can reach the panel without leaving line selection**: an operator still lands on the line-selection screen (their primary screen) after login. When they've been granted any tab in the **Settings → Access** matrix, a **Panel** link appears in the operator top bar that opens the panel at their first accessible tab, and the panel sidebar lists **Lines** first so they can always jump back to the production floor. Operators with no granted tab see no Panel link.
