...
### Changed
- **Operators granted admin tabs now land in the admin panel**: when a non-admin role (e.g. Operator) is given any tab in the **Settings → Access** matrix, members of that role are taken to their first accessible tab after login instead of the operator line-selection screen — so the sidebar with their granted tabs actually appears. Operators with no granted tab keep going to line selection as before.
