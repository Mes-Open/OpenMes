...
### Changed
- **Operators granted admin tabs can reach them without leaving line selection**: an operator still lands on the line-selection screen (their primary screen) after login. When they've been granted any tab in the **Settings → Access** matrix, the operator screen gains a left **sidebar** listing those tabs — with **Lines** (back to line selection) first — so they can open the granted panel pages directly; the panel's own sidebar likewise lists **Lines** first, so navigation is consistent both ways. Operators with no granted tab keep the plain full-screen operator view.
