...
### Added
- **Monorepo: mobile app merged in**: the Expo/React Native operator app now lives in `mobile/` (squash-imported from the OpenMes-mobile repo), so web and mobile ship from one repository.
- **`@openmes/ui` shared design system** (`packages/ui/`, "Geist White"): design tokens plus ~30 components consumed by both the web app (Vite, `.web.jsx` twins styled with Tailwind `om-*` theme tokens) and the mobile app (Metro, `.native.tsx` twins) — buttons, form controls, status pills/badges, menus/dropdowns, inline alerts, toasts/modals/confirm dialogs, a TanStack-Table-powered `DataTable` (`@openmes/ui/table`: column filters, multi-sort, visibility, resizing, sticky header, selection, pagination) and a native-only kit (`@openmes/ui/native`: bottom/action sheets, snackbar, FAB, swipe rows, steppers, wheel picker, app bars, drawer). Geist + Geist Mono fonts wired on both platforms. Dev galleries: `/dev/components` (web, local env) and `app/dev/components` (mobile, dev builds).
- **First Geist White pages**: Admin → Product Types index (web) and the Work Orders tab (mobile) restyled onto the new system as the migration pilots.

### Changed
- **Geist White rollout (full UI restyle, light-only v1)**: the entire web frontend now renders the new design system — admin sidebar/chrome (`AppLayout`), the config-driven CRUD engine (`ResourceTable`/`ResourceForm`, covering ~50 admin pages), operator floor screens (Queue, Workstation, Work-Order detail, Select Line), admin & supervisor dashboards, shift handover, settings, onboarding, packaging station, inspections, auth pages, and the long tail of admin pages (token codemod). Native `window.confirm` prompts replaced with the design system's `ConfirmDialog`; statuses unified on `StatusPill`. On mobile, all shared `components/ui/*` and the `Colors` palette were remapped onto the design tokens, retheming every screen. `dark:` variants were removed from restyled surfaces — the dark shop-floor variant returns later via token theming (the theme toggle still works but both schemes render Geist White for now).
